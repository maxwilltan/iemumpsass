...
// ------------------------------------------------------------
// One-time migration: old browser-only content -> Supabase cloud
// ------------------------------------------------------------
const importLocalDataButton = document.getElementById('importLocalDataButton');
...
function readLegacyIndexedDbEvents() {
  return new Promise(resolve => {
    if (!window.indexedDB) return resolve([]);
    const request = indexedDB.open('iemUmpsaWebsiteDB', 1);
    request.onerror = () => resolve([]);
    request.onupgradeneeded = () => {
      // A brand-new empty DB means there is nothing legacy to import.
    };
    request.onsuccess = () => {
...
async function importLegacyBrowserData() {
  const confirmed = window.confirm(
    'Import content saved in THIS browser into the shared cloud? Existing cloud records with the same IDs will be updated.'
  );
  if (!confirmed) return;

  importLocalDataButton.disabled = true;
  importLocalDataButton.textContent = 'Importing...';

  try {
    const legacyEvents = await readLegacyIndexedDbEvents();
    const legacyCommittee = readLegacyLocalArray('iemUmpsaCommitteeDataV1');
    const legacyGallery = readLegacyLocalArray('iemUmpsaGalleryDataV1');

    let imported = 0;

    for (const event of legacyEvents) {
      await IEMEventStore.saveEvent(event);
      imported += 1;
    }

    for (const member of legacyCommittee) {
      await IEMCommitteeStore.saveMember(member);
      imported += 1;
    }

    for (const album of legacyGallery) {
      await IEMGalleryStore.saveAlbum(album);
      imported += 1;
    }
    ...
    showToast(imported
      ? `${imported} browser records imported to the shared cloud.`
      : 'No old browser-only content was found on this browser.');
  } catch (error) {
    console.error(error);
    showToast(error.message || 'Browser data import failed.');
  } finally {
    importLocalDataButton.disabled = false;
    importLocalDataButton.textContent = 'Import browser data ☁';
  }
}

importLocalDataButton?.addEventListener('click', importLegacyBrowserData);
